      ...
      <div class="section" @click=${()=>this._moreinfo(o.entity)}>   
        ...
        <div class="pool-monitor-container">
          ...
          `:T`
            ...
            ${o.pct_min!==o.pct_cursor?T`<div class="cursor-text" style="border-${o.side_align}: 5px solid ${e.colors.hi_low}; text-align:${o.side_align};background-color:transparent ;${o.side_align}: ${o.pct_min}%;"></div>`:""}
            ${o.pct_max!==o.pct_cursor?T`<div class="cursor-text" style="border-${o.side_align}: 5px solid ${e.colors.hi_low}; text-align:${o.side_align};background-color:transparent ;${o.side_align}: ${o.pct_max}%;"></div>`:""}
          `}
        </div>
    ...
    `}static _moreinfo(e){const o=new Event("hass-more-info",{bubbles:true,composed:true});o.detail={entityId:e};document.querySelector("home-assistant").dispatchEvent(o)}}var P=P||Object.getPrototypeOf(customElements.get("ha-panel-lovelace"));var C=P.prototype.html;console.info(`%c POOL-MONITORING-CARD %c ${_} `,f.title,f.version);class PoolMonitorCard extends P{static cardType=x.cardType;static cardName=x.cardName;static cardDescription=x.cardDescription;static get properties(){return{hass:{},config:{}}}static styles=$;constructor(){super()}render(){const e=this.getConfig();const o=this.processData();console.log("Processed data:",o);const r=e.display.compact?cardContent.generateCompactBody:cardContent.generateBody;if(!o||Object.keys(o).length===0)return C`
      ...
      <div id="pool-monitor-card">
        ...
        ${Object.values(o).map((o=>{if(o?.invalid)return C`
            ...
            `;return r(e,o)}))}
      </div>`}processData(){const e={};const o=this.getConfig();Object.entries(o.sensors).forEach((([o,r])=>{const a=Array.isArray(r)?r:[r];a.forEach(((r,a)=>{const i=`${o}_${a+1}`;e[i]=this.calculateData(o,r.name||`${o} ${a+1}`,r.entity,r.min,r.max,r.setpoint,r.step,r.unit,r.icon,r.image_url,r.mode,r.override_value,r.override,r.invalid)}))}));return e}getTranslatedText(e,o){const r=this.config?.language||"en";const a=getTranslation(r,e);return formatTranslation(a,o)}calculateData(e,o,r,a,i,s,l,n,c,d,p,u,m,g){const h={};const v=this.getConfig();const y=getSensorConfig(e)||{};h.name=e;h.invalid=g;h.mode=p;h.title=v.display.show_names?o:C`&nbsp;`;if(!v.display.show_icons)h.hide_icon=true;else{const o=c||"";const r=d||"";if(o==="hide")h.hide_icon=true;else if(r){h.is_mdi=false;h.img_src=r}else if(o&&typeof o==="string"&&o.startsWith("mdi:")){h.is_mdi=true;h.mdi_icon=o}else h.img_src=`https://raw.githubusercontent.com/wilsto/pool-monitor-card/master/resources/${e}.png`}if(!this.hass||!this.hass.states||!this.hass.states[r]){console.warn(`Entity not found: ${r}`);h.value=null;h.entity=r;return h}h.value=parseFloat(this.hass.states[r].state);h.entity=r;if(v.display.show_last_updated)h.last_updated=this.timeFromNow(this.hass.states[r].last_updated);h.unit=v.display.show_units?n||y.unit||"":"";if(m)h.value=u||y.override;h.min_value=a!==void 0&&this.hass.states[a]&&!isNaN(parseFloat(this.hass.states[a].state))?parseFloat(this.hass.states[a].state):h.value;h.max_value=i!==void 0&&this.hass.states[i]&&!isNaN(parseFloat(this.hass.states[i].state))?parseFloat(this.hass.states[i].state):h.value;s=s!==void 0?parseFloat(s):y.setpoint!==void 0?parseFloat(y.setpoint):h.value;l=l!==void 0?parseFloat(l):y.step!==void 0?parseFloat(y.step):.1;const _=Math.max(this.countDecimals(s),this.countDecimals(l));if(h.value)h.value=parseFloat(this.hass.states[r].state);h.setpoint=s;h.setpoint_class=[(s-2*l).toFixed(_),(s-l).toFixed(_),s.toFixed(_),(s+l).toFixed(_),(s+2*l).toFixed(_)];h.separator=v.show_labels?"-":"";h.color="transparent";if(Number(h.value)<Number(h.setpoint_class[0])){h.state=v.show_labels?this.getTranslatedText("state.1"):"";h.color=v.colors.warn}else if(Number(h.value)>=Number(h.setpoint_class[0])&&Number(h.value)<Number(h.setpoint_class[1])){h.state=v.show_labels?this.getTranslatedText("state.2"):"";h.color=v.colors.low}else if(Number(h.value)>=Number(h.setpoint_class[1])&&Number(h.value)<Number(h.setpoint_class[2])){h.state=v.show_labels?this.getTranslatedText("state.3"):"";h.color=v.colors.normal}else if(Number(h.value)>=Number(h.setpoint_class[2])&&Number(h.value)<Number(h.setpoint_class[3])){h.state=v.show_labels?this.getTranslatedText("state.4"):"";h.color=v.colors.normal}else if(Number(h.value)>=Number(h.setpoint_class[3])&&Number(h.value)<Number(h.setpoint_class[4])){h.state=v.show_labels?this.getTranslatedText("state.5"):"";h.color=v.colors.low}else if(Number(h.value)>=Number(h.setpoint_class[4])){h.state=v.show_labels?this.getTranslatedText("state.6"):"";h.color=v.colors.warn}h.progressClass=e==="temperature"?"progress-temp":"progress";h.pct=Math.max(0,Math.min(98.5,Math.max(0,h.value-(s-3*l))/(6*l)*.85*100+13)).toFixed(0);h.pct_min=Math.max(0,Math.min(98.5,Math.max(0,h.min_value-(s-3*l))/(6*l)*.85*100+13)).toFixed(0);h.pct_max=Math.max(0,Math.min(98.5,Math.max(0,h.max_value-(s-3*l))/(6*l)*.85*100+13)).toFixed(0);h.pct_marker=h.value>h.setpoint?h.pct-13:h.pct-5;h.side_align=h.value>s?"right":"left";h.pct_cursor=h.value>s?100-parseFloat(h.pct):parseFloat(h.pct)-2;h.pct_state_step=h.value>s?105-parseFloat(h.pct):parseFloat(h.pct)+5;h.pct_min=h.value>s?100-parseFloat(h.pct_min):parseFloat(h.pct_min)-2;h.pct_max=h.value>s?100-parseFloat(h.pct_max):parseFloat(h.pct_max)-2;return h}countDecimals(e){if(e===void 0||e===null)return 0;if(Math.floor(e)===e)return 0;const o=e.toString();if(o.includes("."))return o.split(".")[1].length||0;return 0}timeFromNow(e){const o=new Date(e);const r=Date.now()-o.getTime();const t=(e,o)=>{let r=o==1?"":"s";let a=this.getTranslatedText(`time.${e}`);a=a.replace("{"+e+"}",o);a=a.replace("{plural}",r);return a};const a=Math.floor(r/6e4);const i=Math.floor(a/60);const s=Math.floor(i/24);if(a<1)return t("seconds",0);if(a<60)return t("minutes",a);if(i<24)return t("hours",i);return t("days",s)}getConfig(){return this.config}setConfig(e){const o={display:getDisplayConfig(),colors:getColorConfig()};const r={...e,display:{...o.display,...e.display||{}},colors:{...o.colors,...e.colors||{}},sensors:{}};if(!e.sensors){console.warn(`Legacy configuration detected, please move sensors under "sensors" key`,{config:e});throw new Error('Legacy configuration detected. Please update your setup to define all sensors under the "sensors" key as required by the version 2.0 of the card.')}Object.entries(e.sensors).forEach((([o,a])=>{const i=getSensorConfig(o);const s=Array.isArray(a)?[...a]:[{...a}];if(s.length===0)throw new Error(`Empty sensor array for ${o}`);const l=s.map((e=>({...i,...e})));l.forEach(((r,a)=>{if(!r.entity)throw new Error(`Missing entity for ${o}[${a}]`);if(!h.includes(o)){console.warn(`Unsupported sensor type: ${o}`,{sensorType:o,supportedSensors:h,config:e,sensorConfig:r});r.invalid=true}else r.invalid=false}));r.sensors[o]=l}));this.config=r}static _moreinfo(e){const o=new Event("hass-more-info",{bubbles:true,cancelable:false,composed:true});o.detail={entityId:e};document.querySelector("home-assistant").dispatchEvent(o)}}customElements.define("pool-monitor-card",PoolMonitorCard)})();
